...
- Boot-log probe (operator, 2026-09-23): `dmesg | grep -iE 'gt9271|hym8563'` first came back **zero lines** (the touch driver logs as `Goodix-TS` — neither grep term matches it — and the ring buffer had wrapped). Then `sudo grep -iE 'goodix|12-0014|hym8563|15-0051' /var/log/kern.log` (the on-disk boot log) settled both, verbatim (2026-09-20T16:04:51 — the flash/setup date, same failure at every boot since):
  ```
  ...
  ```
  Both probes **ran and failed**: the gt9271's GPIOs were acquired (the driver's "Failed to get %s GPIO" error is absent), its dummy-regulator fallbacks are non-fatal, and the **chip version-register read at `0x8140` fails with -22 (EINVAL)**; the hym8563's init transaction fails with the same errno. Corroborated by the WS1 raw `i2cdetect` grids — **bus 12 and bus 15 both all `--`** (no quick-probe ACK at `0x14`/`0x51`). Mechanism: **both HAT chips are non-responsive on the wire** (absent, unpowered, held in reset, or the I2C lines not where the DT expects) — a **hardware/wiring-level** finding, not a driver/compatible/DT-config issue; every OS-side precondition is in place. (Collateral to the power path; recorded in `touch-gt9271.md` and `rtc-hym8563.md`.)
