...
- Boot-log probe (operator, 2026-09-23): `sudo dmesg | grep -iE 'gt9271|hym8563'` → **zero lines**. Inconclusive for the touch chip (its `goodix_ts` driver's log messages contain no `gt9271` substring — verified against the module's strings — so the grep would not match regardless of the kernel's log-prefix format); also inconclusive for the RTC chip because the ring buffer (~12.5 h uptime) has likely wrapped past both chips' boot-time I2C probe messages. Definitive next evidence: `grep -iE 'goodix|12-0014|hym8563|15-0051' /var/log/kern.log` (world-readable on Armbian, full boot log — persists past ring-buffer wrap).
